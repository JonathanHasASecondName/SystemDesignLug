...
def calculate_compliance_b(youngs_mod,  D_fi, d_sha, L_sha):
    L_hsub = 0.4 * D_fi
    L_engsub = 0.33 * D_fi
    L_nsub =  0.4 * D_fi
...
def calculate_in_plane_loads(alpha_c, alpha_b, delta_T, youngs_mod_fastener, area_sm, youngs_mod_clam, D_fi, D_fo, d_sha, t, L_sha):
    compliance_b = calculate_compliance_b(youngs_mod_fastener, D_fi, d_sha, L_sha)
    compliance_a = calculate_compliance_a(t, youngs_mod_clam, D_fo, D_fi)
    force_ratio = calculate_force_ratio(compliance_a, compliance_b)
    area_sm = (math.pi * (d_sha) ** 2 )/4
    return (alpha_c-alpha_b)*delta_T*youngs_mod_fastener*area_sm*(1-force_ratio)
